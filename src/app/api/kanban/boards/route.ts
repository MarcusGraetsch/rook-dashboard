        ...
        .map((col: Column) => ({
          ...
          tasks: tasks.filter((task: any) => task.column_id === col.id).map((task: any) => {
            ...
            return {
              ...
              workflow_stage: canonical?.workflow_stage || null,
              artifacts: Array.isArray(canonical?.artifacts) ? canonical.artifacts : [],
              retry: canonical?.retry || null,
              parent_task: canonical?.parent_task || null,
              child_tasks: Array.isArray(canonical?.child_tasks) ? canonical.child_tasks : [],
